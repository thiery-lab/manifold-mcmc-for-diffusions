"""Run experiment inferring FitzHugh-Nagumo model (noisy obs.) parameters with HMC."""
...
output_dir = os.path.join(args.output_root_dir, "fhn_noisy_hmc", dir_name)
...
for c in range(args.num_chain):
    ...
    state = sde.mici_extensions.find_initial_state_by_linear_interpolation(
        ...
        generate_z=fhn.generate_z,
        generate_x_0=fhn.generate_x_0,
        generate_σ=fhn.generate_σ
        if variable_σ
        else lambda u: args.observation_noise_std,
        forward_func=fhn.forward_func,
        obs_func=fhn.obs_func,
        y_seq=y_seq,
