...
parser.add_argument(
    '--output-root-dir', default='experiments', 
    help='Root directory to make experiment output subdirectory in'
)
parser.add_argument(
    '--num-steps-per-obs', type=int, default=25,
    help='Number of discrete time steps per interobservation interval'
)
parser.add_argument(
    '--num-obs', type=int, default=100,
    help='Number of observation times'
)
parser.add_argument(
    '--num-obs-per-subseq', type=int, default=5,
    help='Average number of observations per blocked subsequence'
)
parser.add_argument(
    '--splitting', choices=('standard', 'gaussian'), default='standard',
    help=(
...
forward_func = symnum.numpify_func(
    ...
    (dim_z,), (dim_x,), (dim_v,), None, numpy_module=jnp)
...
obs_interval = 0.2
num_steps_per_obs_data = 1000
δ = obs_interval / num_steps_per_obs_data
z_true = onp.array([0.3, 0.1, 1.5, 0.8]) # [σ, ϵ, γ, β]
x_0_true = onp.array([-0.5, -0.6])
v_seq_true = rng.standard_normal((args.num_obs * num_steps_per_obs_data, dim_v))
...
y_seq = onp.asarray(
    generate_from_model(z_true, x_0_true, v_seq_true, δ, num_steps_per_obs_data)
)

# Set up Mici objects

system = sde.mici_extensions.ConditionedDiffusionConstrainedSystem(
    obs_interval, args.num_steps_per_obs, args.num_obs_per_subseq, y_seq, 
    dim_u, dim_x, dim_v, forward_func, generate_x_0, generate_z, 
    obs_func, use_gaussian_splitting=args.splitting =='gaussian'
)

projection_solver = (
    sde.mici_extensions.jitted_solve_projection_onto_manifold_newton
    if args.projection_solver == 'newton' else
    sde.mici_extensions.jitted_solve_projection_onto_manifold_quasi_newton
)
...
integrator = mici.integrators.ConstrainedLeapfrogIntegrator(
    system, n_inner_step=args.num_inner_h2_step,
...
def trace_func(state):
    ...
    return {
        'σ': z[0], 'ϵ': z[1], 'γ': z[2], 'β': z[3], 'x_0': x_0, 'q': q, 
        **call_counts
    }
...
final_states, traces, stats = sampler.sample_chains_with_adaptive_warm_up(
    args.num_warm_up_iter, args.num_main_iter, init_states, 
    trace_funcs=[trace_func],
    adapters={'integration': [step_size_adapter]},
    memmap_enabled=True, memmap_path=output_dir,
    monitor_stats=[('integration', 'accept_stat'), 
...
print(arviz.summary(traces, var_names=['σ', 'ϵ', 'γ', 'β', 'x_0']))
